...
| **Bake FAILED: `IndexOutOfRangeException` in `MeshCollection.ImportMeshes`** (multi-material animated model) | The multi-material animated path **rebuilds** the skinned mesh (`Instantiate` + submesh-merge), and Amplitude's skeleton importer needs **tangents** on that rebuilt mesh — with them absent it indexes an empty tangent array and throws. Fixed: the baker keeps tangents for multi-material FBX imports (`CalculateMikk`) and calls `RecalculateTangents()` on the merged mesh. **The mesh-slimming optimizations (weld pre-pass, tangent strip) are single-material-only** for this reason. If you hit this after a baker change, check that multi-material imports still get tangents. |
