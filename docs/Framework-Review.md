...
| 07-18 | **First humanoid character + rotation-for-animated + enforced window ownership**: the Combine soldier (62-bone rig, `Idle1`) replaces the DroneSquadFPV Humvee — standing, right-sized, turning with movement, idling in-game. En route, three systemic fixes: (1) **Rotation is now REAL for animated models** — applied to the rig in the Blender step and **folded into the data** (`transform_apply`: vertices + bone rests, identity nodes; object-level rotation alone is dropped by downstream readers). Raw glTF rigs can ship a compensating armature-node rotation (soldier: -90°X) that made them bake lying down — now correctable (soldier needs `90,0,0`); `deploy_convert`-prepared rigs (howitzer) are already clean. Object-level anim fcurves are stripped so they can't re-assert orientation. ⚠ preview orientation ≠ game orientation (fixed stand-up flips) — dial via quick bakes, judge in-game. (2) **Enforced field ownership across Factory/Animation Lab** — bake/save rebases on the freshest registry entry, each window contributes only its own fields (stale-copy bakes had clobbered Fix-100×, then rotation+size). (3) **BoneRotation layer discovery** — the game turns pawns through the procedural `PawnEntry.BoneRotation0-3` layer; blanket-clearing it (the old barrel-twist fix) froze the soldier's facing. Now cleared only for artillery-behavior models. OPEN (one subsystem): drone projectile visual lost on attack (pose hook stomps the throw anim that carries FireProjectile — fix: pass the game's pose through when it isn't idle/move), hood stretch after turning (junk BoneRotation slots with invalid bone index + 1500°+ angles on this donor — needs the layer's writer decompiled), temp `[Uni][facing]` diagnostic stays in. |
